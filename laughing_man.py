...
# OpenCV (highgui) and MediaPipe pull in Qt / TFLite native code at import time.
# Apply defaults before those imports so stderr is not flooded on typical Linux
# (missing Wayland plugin in cv2's Qt, font paths, and verbose native logs).


def _apply_laughing_man_runtime_env() -> None:
    """
    ...
    """
    # TensorFlow Lite / glog (MediaPipe): keep only fatal native messages.
    os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "3")
    os.environ.setdefault("GLOG_minloglevel", "3")

    # OpenCV Linux builds usually ship Qt with X11 (xcb), not Wayland; under a
    # Wayland session Qt may probe for a missing wayland plugin and print errors.
    if sys.platform.startswith("linux"):
        ...
        for font_root in ("/usr/share/fonts", "/usr/local/share/fonts"):
            if os.path.isdir(font_root):
                ...
                break
        # Fewer Qt category lines; some plugin messages still go to stderr.
...
_apply_laughing_man_runtime_env()
